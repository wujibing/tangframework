...
class Html implements ITemplate
{
	...
	protected function compile($templateFile,$compileFile)
	{
		...
		$content = preg_replace('$\{StorageUrl\((.+?)\)\}$', '<?php echo \Tang\Storage\StorageService::getUrl(\\1);?>', $content);
		$content = preg_replace('$\{Form-\>(.+?)\((.+?)\)\}$', '<?php echo \Tang\Util\Form::createByFile(\\1,\\2);?>', $content);
		$content = preg_replace('$\{Form\.(.+?)\((.+?)\)\}$', '<?php echo \Tang\Util\Form::\\1(\\2);?>', $content);
		...
		$content = preg_replace('$\{Lang-\>(.+?)}$', '<?php echo \CkFramework\I18n\Lang::GetLang(\'\\1\');?>', $content);
		$content = preg_replace('$\{Lang\.(.+?)}$', '<?php echo \CkFramework\I18n\Lang::GetLang(\\1);?>', $content);
        $content = preg_replace('$\{Url\((.+?)\)}$', '<?php echo \Tang\Services\RouterService::getService()->driver(\'web\')->createUrl(\\1);?>', $content);
		...
		$content = preg_replace('$\{Config-\>(.+?)}$', '<?php echo \Tang\Services\ConfigService::get(\'\\1\');?>', $content);
		$content = preg_replace('$\{Config\.(.+?)}$', '<?php echo \Tang\Services\ConfigService::get(\\1);?>', $content);
