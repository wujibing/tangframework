...
class Captcha
{
    ...
    public function create($id)
    {
        ...
        $fontSize = $this->width / $length * $scale + 8;
        ...
        for ($i=0;$i<$this->codeLength;$i++)
        {
            ...
            $box = $image->draw($text);
            $image->draw(new Line($x-mt_rand(1,20),$lineY,$x+$fontSize,$lineY,$textColor,$lineSize));
            $image->draw(new Line($x-mt_rand(1,20),$box[4],$x+$fontSize,$y,$textColor,$lineSize));
            $x += $box[2];
        }
        SessionService::getService()->set('Captcha'.$id,$captchaCode);
        $image->draw(new Wave(mt_rand(15,40),mt_rand(7,40)));
